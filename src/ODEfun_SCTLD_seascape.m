...
%% Calculate derivatives - MASS CONSERVING VERSION
% Susceptible populations decrease only where disease is active
...
% New infections only occur at active sites
new_infections_LS = zeros(length(I_LS), 1);
...
% CRITICAL: Recovery happens EVERYWHERE there is infection, not just at active sites
% This ensures mass conservation even when sites flicker in/out of "active" status
dI_LS = new_infections_LS - g_LS * I_LS;
dI_MS = new_infections_MS - g_MS * I_MS;
dI_HS = new_infections_HS - g_HS * I_HS;

% Recovery (removal) happens everywhere there is infection
dR_LS = g_LS * I_LS;
dR_MS = g_MS * I_MS;
dR_HS = g_HS * I_HS;
