...
if DO_PREPROCESSING
    ...
    for event_idx = 1:n_events
        ...
        for idx = 1:n_relevant_files
            ...
            try
                ...
                lon_cells{idx} = lon_flat;
                lat_cells{idx} = lat_flat;
                src_cells{idx} = src_flat;
        ...
        elapsed = toc;
        fprintf('  Collected %d particle positions in %.1f seconds\n', ...
            numel(all_lon), elapsed);
        
        % Save to file with descriptive name
        event_data = struct();
        event_data.event_idx = event_idx;
        event_data.target_date = target_date;
        event_data.calendar_date = calendar_date;
        event_data.all_lon = all_lon;
        event_data.all_lat = all_lat;
        event_data.all_source_idx = all_source_idx;
        
        save(event_filename, 'event_data', '-v7.3');
        fprintf('  Saved to: %s\n', event_filename);
    end
    ...
    load(event_filename, 'event_data');
    
    fprintf('  Julian date: %.2f\n', event_data.target_date);
    fprintf('  Calendar date: %s\n', datestr(event_data.calendar_date, 'dd-mmm-yyyy HH:MM:SS'));
    fprintf('  Total particles: %d\n', numel(event_data.all_lon));
    
    % Extract data
    all_lon = event_data.all_lon;
    all_lat = event_data.all_lat;
    all_source_idx = event_data.all_source_idx;
    target_date = event_data.target_date;
    calendar_date = event_data.calendar_date;
end
