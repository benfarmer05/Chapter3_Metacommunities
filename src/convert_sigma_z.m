...
u_new(u_new == 0) = NaN;
for i = 1:size(u_new,1) %row 364
    for j = 1:size(u_new,2) %column 543
        ...
        %original version, clipping seafloor mask by rho
        vec = interp1(xinterp, squeeze(u(i,j,:)), zlevels, 'linear');

        for k = 1:numlevels %test: k = 16
            % if zlevels(k) > xinterp(end) %testing version, clipping the mask by grid
            if zlevels(k) > h(i,j) %this was the original version, clipping the mask by rho
...
for i = 1:size(w_new,1)
    for j = 1:size(w_new,2)
        ...
        % %%
        % x = abs(squeeze(rhodepths(i,j,:))); %the known depths. x = abs(squeeze(udepths(row,col,:)));rhodepths
        % v = squeeze(w(i,j,:)); %the known values of interest (current speed, temp, salinity) at those depths. v = squeeze(t(row,col,:));
...
% % for i = 1:numlevels
% %     u_Agrid(:,:,i) = uv2rho(u_new(:,:,i), 'u'); %interpolate u_new (535x716) to rho-grid (536x716)
% %     v_Agrid(:,:,i) = uv2rho(v_new(:,:,i), 'v'); %interpolate v_new (536x715) to rho-grid (536x716)
% % end
% u_Agrid = uv2rho(u_new, 'u'); %interpolate u_new (535x716) to rho-grid (536x716)
% v_Agrid = uv2rho(v_new, 'v'); %interpolate v_new (536x715) to rho-grid (536x716)
% u_new = u_Agrid;
% v_new = v_Agrid;
