...
use_group_denominator <- TRUE  # TRUE = divide by group total, FALSE = divide by site total
...
plot_totals <- function(data, scen_label) {
  ...
  p_infected <- ggplot(I_long, aes(x = date, y = infected, group = site)) +
    geom_line(alpha = linealpha, color = compartment_colors["Infected"], linewidth = linewidths) +
  ...
  p_removed <- ggplot(R_long, aes(x = date, y = removed, group = site)) +
    geom_line(alpha = linealpha, color = compartment_colors["Removed"], linewidth = linewidths) +
    scale_y_continuous(labels = scales::comma) +
    scale_x_date(date_labels = "%b-%Y") +
    labs(x = NULL, y = "Coral cover (%)") +
    theme_classic(base_family = "Georgia") +
  ...
  legend_data <- data.frame(
    date = rep(as.Date("2019-01-01"), 2),
    value = c(1, 1),
    compartment = c("Infected", "Removed")
  )
  
  p_legend <- ggplot(legend_data, aes(x = date, y = value, color = compartment)) +
    geom_line(linewidth = linewidths * 1.5) +
    scale_color_manual(values = compartment_colors, name = NULL) +
    theme_void() +
    theme(legend.position = "right", legend.text = element_text(size = textsize, family = "Georgia"))
  
  legend <- get_legend(p_legend)
  
  combined_plot <- (p_infected / p_removed) + plot_layout(heights = c(1, 1), axes = "collect_y")
  final_plot <- combined_plot | legend
  final_plot <- final_plot + plot_layout(widths = c(10, 1.5))
...
plot_by_group <- function(data, scen_label) {
  I_by_group <- data$I_by_group %>% mutate(infected = infected * 100)
  R_by_group <- data$R_by_group %>% mutate(removed = removed * 100)
  ...
  combined_plot <- (p_infected_group / p_removed_group) +
    ...
    theme(legend.position = "right", legend.box.spacing = unit(0, "cm"))
