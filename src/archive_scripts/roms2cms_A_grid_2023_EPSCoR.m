...
for i = 1:length(files) %loop through each file
    ...
    for j = 1:3 %loop through each hour in each file. there are three hours per file
        ...
        ncwrite(cms_fname, 'zs', s_new)
                ...
                % STOPPING POINT - 1 SEP 2023. I need to check that the regridding
        % and restructuring of the w,t,s for-loop is working properly.
        % there is an issue right now where the regridding seems to be
        % messing up u & v's landmask. definitely an NA issue, but no idea
        % how to solve it. maybe need to force u & v to have NA values in
        % the exact landmask coordinates as rho, but somehow only
        % interpolate around that landmask
        %
        % or...could chop off from 536 to 535 for v, and also chop off from 716
        % to 715 for u? but that would also mean rho needs to get chopped
        % off in both directions, and I don't know if any of that works
