  ...
  # palette_R_rel <- "turbo"
  # palette_I_abs <- "inferno"
  # palette_I_rel <- "inferno"
  # palette_R_abs <- "viridis"
  # palette_R_rel <- "viridis"
  # palette_I_abs <- "rocket"
  # palette_I_rel <- "rocket"
  # palette_R_abs <- "mako"
  # palette_R_rel <- "mako"
  palette_I_abs <- "turbo"
  palette_I_rel <- "turbo"
  palette_R_abs <- "plasma"
  palette_R_rel <- "plasma"
  ...
  p_I_abs <- ggplot() +
    ...
    geom_spatvector(data = land_vect, fill = "gray90", color = "gray40", linewidth = 0.05) +
  ...
  ################################## export csv for Table 4.1, summarizing infections and loss ##################################
  
  # NOTE - relative values, for group-specific entries, area relative to N_sg
  ...
  final_I_LS <- I_LS[num_days, ]
  final_I_MS <- I_MS[num_days, ]
  final_I_HS <- I_HS[num_days, ]
  final_R_LS <- R_LS[num_days, ]
  final_R_MS <- R_MS[num_days, ]
  final_R_HS <- R_HS[num_days, ]
  
  # Get final R_total and I_total directly
  final_R_total <- R_total[num_days, ]
  final_I_total <- I_total[num_days, ]
  
  # Calculate initial N for each group (S + I + R at t=1)
  N_LS <- S_LS[1, ] + I_LS[1, ] + R_LS[1, ]
  N_MS <- S_MS[1, ] + I_MS[1, ] + R_MS[1, ]
  N_HS <- S_HS[1, ] + I_HS[1, ] + R_HS[1, ]
  
  # Relative values for totals (divided by site total N_site)
  final_I_total_rel <- final_I_total / N_site
  final_R_total_rel <- final_R_total / N_site
  
  # Relative values for groups (divided by group-specific N)
  final_I_LS_rel <- final_I_LS / N_LS
  final_I_MS_rel <- final_I_MS / N_MS
  final_I_HS_rel <- final_I_HS / N_HS
  final_R_LS_rel <- final_R_LS / N_LS
  final_R_MS_rel <- final_R_MS / N_MS
  final_R_HS_rel <- final_R_HS / N_HS
  
  # Replace Inf/NaN with 0
  final_I_total_rel[!is.finite(final_I_total_rel)] <- 0
  final_R_total_rel[!is.finite(final_R_total_rel)] <- 0
  final_I_LS_rel[!is.finite(final_I_LS_rel)] <- 0
  final_I_MS_rel[!is.finite(final_I_MS_rel)] <- 0
  final_I_HS_rel[!is.finite(final_I_HS_rel)] <- 0
  final_R_LS_rel[!is.finite(final_R_LS_rel)] <- 0
  final_R_MS_rel[!is.finite(final_R_MS_rel)] <- 0
  final_R_HS_rel[!is.finite(final_R_HS_rel)] <- 0
  
  # Filter: sites with >5% total relative removal
  threshold <- 0.05  # 5% as proportion
  affected_idx <- which(final_R_total_rel > threshold)
  
  cat("\n========================================\n")
  cat("Sites with >5% total relative removal:", length(affected_idx), 
      "of", length(final_R_total_rel), "total sites\n")
  cat("========================================\n\n")
  
  # Subset all variables to affected sites only
  final_I_total_sub <- final_I_total[affected_idx]
  final_R_total_sub <- final_R_total[affected_idx]
  final_I_LS_sub <- final_I_LS[affected_idx]
  final_I_MS_sub <- final_I_MS[affected_idx]
  final_I_HS_sub <- final_I_HS[affected_idx]
  final_R_LS_sub <- final_R_LS[affected_idx]
  final_R_MS_sub <- final_R_MS[affected_idx]
  final_R_HS_sub <- final_R_HS[affected_idx]
  
  final_I_total_rel_sub <- final_I_total_rel[affected_idx]
  final_R_total_rel_sub <- final_R_total_rel[affected_idx]
  final_I_LS_rel_sub <- final_I_LS_rel[affected_idx]
  final_I_MS_rel_sub <- final_I_MS_rel[affected_idx]
  final_I_HS_rel_sub <- final_I_HS_rel[affected_idx]
  final_R_LS_rel_sub <- final_R_LS_rel[affected_idx]
  final_R_MS_rel_sub <- final_R_MS_rel[affected_idx]
  final_R_HS_rel_sub <- final_R_HS_rel[affected_idx]
  ...
  summary_table <- data.frame(
    Compartment = c("Infected", "", "", "", "", "", "", "",
                    "Removed", "", "", "", "", "", "", ""),
    Group = rep(c("Low", "", "Moderate", "", "High", "", "All groups", ""), 2),
    Measure = rep(c("Abs.", "Rel."), 8),
    Mean = fmt(c(
      mean(final_I_LS_sub) * 100, mean(final_I_LS_rel_sub) * 100,
      mean(final_I_MS_sub) * 100, mean(final_I_MS_rel_sub) * 100,
      mean(final_I_HS_sub) * 100, mean(final_I_HS_rel_sub) * 100,
      mean(final_I_total_sub) * 100, mean(final_I_total_rel_sub) * 100,
      mean(final_R_LS_sub) * 100, mean(final_R_LS_rel_sub) * 100,
      mean(final_R_MS_sub) * 100, mean(final_R_MS_rel_sub) * 100,
      mean(final_R_HS_sub) * 100, mean(final_R_HS_rel_sub) * 100,
      mean(final_R_total_sub) * 100, mean(final_R_total_rel_sub) * 100
    )),
    Median = fmt(c(
      median(final_I_LS_sub) * 100, median(final_I_LS_rel_sub) * 100,
      median(final_I_MS_sub) * 100, median(final_I_MS_rel_sub) * 100,
      median(final_I_HS_sub) * 100, median(final_I_HS_rel_sub) * 100,
      median(final_I_total_sub) * 100, median(final_I_total_rel_sub) * 100,
      median(final_R_LS_sub) * 100, median(final_R_LS_rel_sub) * 100,
      median(final_R_MS_sub) * 100, median(final_R_MS_rel_sub) * 100,
      median(final_R_HS_sub) * 100, median(final_R_HS_rel_sub) * 100,
      median(final_R_total_sub) * 100, median(final_R_total_rel_sub) * 100
    )),
    `75th` = fmt(c(
      p75(final_I_LS_sub) * 100, p75(final_I_LS_rel_sub) * 100,
      p75(final_I_MS_sub) * 100, p75(final_I_MS_rel_sub) * 100,
      p75(final_I_HS_sub) * 100, p75(final_I_HS_rel_sub) * 100,
      p75(final_I_total_sub) * 100, p75(final_I_total_rel_sub) * 100,
      p75(final_R_LS_sub) * 100, p75(final_R_LS_rel_sub) * 100,
      p75(final_R_MS_sub) * 100, p75(final_R_MS_rel_sub) * 100,
      p75(final_R_HS_sub) * 100, p75(final_R_HS_rel_sub) * 100,
      p75(final_R_total_sub) * 100, p75(final_R_total_rel_sub) * 100
    )),
    Max = fmt(c(
      max(final_I_LS_sub) * 100, max(final_I_LS_rel_sub) * 100,
      max(final_I_MS_sub) * 100, max(final_I_MS_rel_sub) * 100,
      max(final_I_HS_sub) * 100, max(final_I_HS_rel_sub) * 100,
      max(final_I_total_sub) * 100, max(final_I_total_rel_sub) * 100,
      max(final_R_LS_sub) * 100, max(final_R_LS_rel_sub) * 100,
      max(final_R_MS_sub) * 100, max(final_R_MS_rel_sub) * 100,
      max(final_R_HS_sub) * 100, max(final_R_HS_rel_sub) * 100,
      max(final_R_total_sub) * 100, max(final_R_total_rel_sub) * 100
    )),
    check.names = FALSE
  )
  ...
  write.csv(summary_table, 
            here("output", "output_figures_tables", "final_state_summary_stats.csv"),
            row.names = FALSE)
  
  cat("\n✓ Summary table saved to output/output_figures_tables/final_state_summary_stats.csv\n")