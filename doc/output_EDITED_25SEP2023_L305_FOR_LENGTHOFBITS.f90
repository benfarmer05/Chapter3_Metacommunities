...
!write output
 ...
 !writes density and diameter, release polygon and pld given by temperature
   write(iunit_traj, &
 '(I8,1x, I8,1x, I12,1x, F8.4,1x, F8.5,1x, F8.4,1x, F12.3,1x, I2,1x, F12.3,1x, I6,1x, F6.1,1x, F10.7)') &
 ...
 ! writes density and diameter
   write(iunit_traj, &
   '(I8,1x, I8,1x, I12,1x, F8.4,1x, F8.5,1x, F8.4,1x, F12.3,1x, I2,1x, F12.3,1x, F6.1,1x, F10.7)') &
   r, n, run_time, nlon, nlat, ndepth, dist, exitCode, relDate, &
   dens_part, diam_part
 ELSE IF (polygon) THEN
 ! writes release polygon
   write(iunit_traj, &
   '(i8,1x, I8,1x, I12,1x, F8.4,1x, F8.5,1x, F8.4,1x, F12.3,1x, I2,1x, F12.3,1x, I6)') &
   r, n, run_time, nlon, nlat, ndepth, dist, exitCode, relDate, &
   poly
 ELSE IF (withibm) THEN
 ! writes distance and reldate
   write(iunit_traj, &
   '(i8,1x,i8,1x,i12,1x,F8.4,1x,F8.5,1x,F8.4,1x,F12.3,1x,i2,1x,f12.3)') &
   r, n, run_time, nlon, nlat, ndepth, dist, exitCode, relDate
 ELSE
 ! only writes default output - time, position and exitCode
   write(iunit_traj,'(i8,1x,i12,1x,F8.4,1x,F8.5,1x,F8.4,1x,i2)') &
